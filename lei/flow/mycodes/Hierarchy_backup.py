...
if __name__ == "__main__":
		...
		for ep in range(Epoch):	
			print("")
			print("sub-agent Epoch:", Epoch)
			# phase of sub-policy.
			for i in range(sub_train_epi):
				
				print("sub-agent episode: ", i)
				global_counter += 1
				state = env.reset()	# 长度即为sub_agent的个数
				...
				[sub_agents[k].summarize(ep_r, i + (ep * sub_train_epi), 'reward') for k in range(Agent_NUM)]
