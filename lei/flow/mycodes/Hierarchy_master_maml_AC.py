...
NUM_TASKS = 5
EDGE_INFLOWS = [500, 550, 600, 650, 700]
...
if __name__ == "__main__":
		...
		NAME = '2x2_600_PPO_Hierarchy_SOFT_MASTER_MAML_AC_try0'

		#*********************
		Epoch = NUM_TASKS * 1
		# sub_train_epi = 1	# 25
		centre_train_epi = 1	# 15	不影响maml训练
		steps = 30	# 200	不影响maml训练
		#*********************
		...
		centre_agent = PPO(s_dim=Agent_NUM, a_dim=Agent_NUM+1, name="centre")	# 用来get梯度，不训练
		# PHI agent 除了训练方式不同，其他都相当于centre agent
		centre_agent_maml_AC = PPO_MAML_AC(s_dim=Agent_NUM, a_dim=Agent_NUM+1, name="centre_maml_AC")
		...
		# master之后需要调用maml参数
		pretrained_ep_master_maml_AC = ''
		#centre_agent.restore_params('master_maml_{}'.format(pretrained_ep_master_maml))
		
		# train
		global_counter = 0	# 记录episode次数
		each_line_path = "collected_data/hierarchy/{}_plot_log_maml_AC.txt".format(NAME)
		test_epoch_path = "collected_data/hierarchy/{}_epoch_log_maml_AC.txt".format(NAME)
		...
		for ep in range(Epoch):
			...
			flow_params = dict(
				...
				exp_tag="grid_0_{}x{}_i{}_multiagent_maml_AC".format(N_ROWS, N_COLUMNS, "EDGE_INFLOW=200~400"),
			...
			for j in range(1):#centre_train_epi
				...
				for step in range(steps):
					...
					centre_agent.experience_store(actions, centre_action, )
					# 训练过程中，maml和原master共享所有的s,a,r
					centre_agent_maml_AC.experience_store(actions, centre_action, )
					...
					if (step + 1) % BATCH == 0 or step == EP_LEN - 1:
						...
						if (ep+1) % 5 == 0:
							# 这步需要5个task的梯度
							print("~~~~~~~~~~~~~~MAML_AC UPADATE !!~~~~~~~~~~~~~~")
							centre_agent_maml_AC.init_maml()
							centre_agent_maml_AC.update()

						centre_agent_maml_AC.empty_buffer()
			...
			# 每10个epoch保存一次参数
			if ep % 10 == 0:
				centre_agent_maml_AC.save_params('master_maml_AC_{}'.format(ep))
				pass
