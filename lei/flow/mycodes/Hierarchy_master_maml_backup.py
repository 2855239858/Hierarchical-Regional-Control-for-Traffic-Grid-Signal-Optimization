...
if __name__ == "__main__":
		...
		NAME = '2x2_600_PPO_Hierarchy_SOFT_MASTER_MAML_try0'

		#*********************
		Epoch = NUM_TASKS * 1
		# sub_train_epi = 1	# 25
		centre_train_epi = 10	# 15	不影响maml训练
		steps = 210	# 200	不影响maml训练
		#*********************
		...
		for ep in range(Epoch):
			...
			for j in range(1):#centre_train_epi
				...
				for step in range(steps):
					...
					centre_agent_maml.experience_store(actions, centre_action, step_r)
			...
			# 每10个epoch保存一次参数
			if ep % 10 == 0:
				centre_agent_maml.save_params('master_maml_{}'.format(ep))
				pass
